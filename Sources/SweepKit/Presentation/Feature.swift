...
public enum Feature: String, CaseIterable, Identifiable, Sendable {
    ...
    /// 검토 목록 위에 붙는 한 문장.
    ///
    /// `summary`(시작 화면용)와 다르다. 저쪽은 "무엇을 찾는 화면인가"를 말하고
    /// 여기는 **"무엇을 기준으로 고르라는 것인가"**를 말한다. 목록을 그냥 던지면
    /// 사용자는 판단 기준 없이 체크박스만 본다.
    public var reviewHint: String {
        switch self {
        case .junk:
            "대부분 빌드하면 다시 생깁니다. 되돌릴 수 없는 심사 제출본은 잠가 두었습니다."
        case .largeFile:
            "설치 파일은 다시 받을 수 있습니다. 촬영 원본과 백업은 내용을 확인한 뒤 고르세요."
        case .duplicate:
            "원본은 항상 남깁니다. 아래는 지워도 같은 파일이 그대로 남아 있는 사본입니다."
        case .smartScan:
