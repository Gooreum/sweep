...
case "${1:-}" in
  ...
  launch)
    ...
    pgrep -f "$PROC_PATTERN" >/dev/null || { echo "앱이 뜨지 않았다" >&2; exit 1; }
    activate || { echo "앱을 앞으로 부를 수 없다" >&2; exit 1; }
    echo OK
    ;;
  ...
  navigate)
    ...
    python3 -c "import time; time.sleep(3)"
    pgrep -f "$PROC_PATTERN" >/dev/null || { echo "재기동 실패: $SCREEN" >&2; exit 1; }
    activate || { echo "앱을 앞으로 부를 수 없다" >&2; exit 1; }
    echo OK
    ;;
  ...
  capture)
    ...
    WID=$(window_id) || { echo "창을 찾을 수 없다" >&2; exit 1; }
    rm -f "$OUT"
